...
const quickActions = [
  { icon: AlertOctagon, label: "Get Help Now", path: "/sos", color: "text-red-500", bg: "bg-red-500/10", border: "border-red-500/20" },
  { icon: Users, label: "Your Contacts", path: "/guardian", color: "text-slate-200", bg: "bg-slate-800/40", border: "border-slate-700/50" },
  { icon: MessageSquare, label: "Safety Chat", path: "/assistant", color: "text-teal-500", bg: "bg-teal-500/10", border: "border-teal-500/20" },
...
const recentActivity = [
  ...
  { icon: Shield, label: "Protection Ended", time: "20m ago", color: "text-slate-400", bg: "bg-slate-800/40" },
  { icon: MapPin, label: "Location Shared", time: "1h ago", color: "text-blue-500", bg: "bg-blue-500/10" },
];
      ...
      <div className="bg-background min-h-screen pb-40">
        <div className="max-w-[1400px] mx-auto px-6 pt-6">
            ...
            <div className="flex items-center justify-between w-full">
                <motion.div initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} className="space-y-2">
                  <h1 className="text-3xl font-black text-slate-100 tracking-tight uppercase" style={{ fontFamily: "Manrope, sans-serif" }}>
                    System Status: <span className="text-teal-500">Active</span>
                  </h1>
                  <p className="text-slate-500 font-black text-[10px] uppercase tracking-[0.3em]">Everything looks safe • Tracking active • Signal strong</p>
               </motion.div>
               <div className="inline-flex items-center gap-3 px-4 py-2 bg-slate-900/40 rounded-2xl border border-slate-800/50 backdrop-blur-md">
                  ...
                  <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Safety Monitoring On</span>
               </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            {[
              { label: "Safety Score", value: "98", unit: "/100", color: "text-teal-500", bg: "bg-teal-500/10", icon: Shield },
              { label: "Trusted Contacts", value: "3", unit: " Active", color: "text-blue-500", bg: "bg-blue-500/10", icon: Users },
              { label: "Live Connection", value: "Strong", unit: "", color: "text-slate-100", bg: "bg-slate-800/40", icon: Radio },
              { label: "Locker Status", value: "Private", unit: "", color: "text-indigo-400", bg: "bg-indigo-500/10", icon: Clock },
            ].map((stat, i) => (
              <motion.div 
                key={stat.label} 
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div className="xl:col-span-2 space-y-6">
               <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="relative rounded-[28px] overflow-hidden bg-gradient-to-br from-[#0f172a] to-[#334155] p-8 shadow-xl">
                  ...
                  <div className="relative z-10">
                    <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-teal-500/20 text-teal-300 border border-teal-500/30 text-[9px] font-black uppercase tracking-widest mb-4">
                       <Shield className="w-3 h-3" /> You are safe
                    </div>
                    <h2 className="text-xl font-bold text-white mb-2 leading-tight">Safety Mode</h2>
                    <p className="text-slate-400 text-[13px] font-medium leading-relaxed mb-6">
                       We are tracking your location and surroundings to keep you safe.
                    </p>
                    <motion.button 
                    ...
                    >
                       Get Help & Chat <Navigation2 className="w-3.5 h-3.5" />
                    </motion.button>
                  </div>
               </motion.div>
               
               <div className="bg-slate-900/40 backdrop-blur-md p-8 rounded-[32px] border border-slate-800/50 shadow-lg">
                  <h3 className="text-[15px] font-black text-slate-100 uppercase tracking-widest mb-6 flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {quickActions.map(({ icon: Icon, label, path, color, bg, border }) => (
                      <motion.button
                        ...
                        className="group bg-slate-950/40 p-6 rounded-[28px] border border-slate-800/50 shadow-lg flex flex-col items-center text-center gap-3 transition-all"
                      >
               ...
               <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-900/40 backdrop-blur-md p-8 rounded-[32px] border border-slate-800/50 shadow-2xl">
                 <div className="flex items-center justify-between mb-8">
                   <div className="flex items-center gap-3">
                     <motion.span animate={{ opacity: [1, 0.4, 1] }} transition={{ repeat: Infinity, duration: 1.5 }} className="w-2 h-2 rounded-full bg-red-500" />
                     <h3 className="text-[14px] font-black text-slate-100 uppercase tracking-[0.15em]">Live Updates</h3>
                   </div>
                   ...
                   {recentActivity.map((item, i) => (
                     ...
                     >
                       ...
                       <div className="flex-1">
                         ...
                         <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.15em]">{item.time} • System checking...</p>
                       </div>
                     </motion.button>
                   ))}
                 </div>
            ...
            <div className="space-y-6">
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-slate-900/40 backdrop-blur-md p-8 rounded-[32px] border border-slate-800/50 shadow-2xl">
                   <div className="flex items-center justify-between mb-6">
                     <h3 className="text-[14px] font-black text-slate-100 uppercase tracking-widest">Active Location</h3>
                   ...
                   <div className="bg-slate-950/50 rounded-2xl p-5 mb-6 border border-slate-800/50">
                      <p className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] mb-2">Where you are</p>
                      <p className="text-slate-100 text-[12px] font-black uppercase tracking-tight leading-relaxed truncate">
                         {locationState.loading ? "Finding you..." : locationState.address || "Sector 18, Noida"}
                      </p>
                   </div>
                   ...
                   <motion.button whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }} onClick={() => navigate("/risk-map")} className="w-full mt-6 py-4 bg-teal-500/5 border border-teal-500/20 text-teal-500 font-black text-[11px] rounded-2xl uppercase tracking-[0.2em] hover:bg-teal-500/10 transition-all cursor-pointer">View Safety Map</motion.button>
                </motion.div>
               ...
               <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-slate-900/40 backdrop-blur-md p-8 rounded-[32px] border border-slate-800/50 shadow-lg">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                       ...
                       <h3 className="text-[14px] font-black text-slate-100 uppercase tracking-widest">Safety Band</h3>
                    </div>
                    <span className="text-[9px] font-black text-teal-500 bg-teal-500/10 px-2 py-0.5 rounded border border-teal-500/20 uppercase tracking-wider">Connected</span>
                  </div>
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => { triggerSOS(); navigate("/sos"); }} className="w-full py-3.5 bg-red-500/10 border border-red-500/20 text-red-500 font-black text-[11px] rounded-xl uppercase tracking-widest hover:bg-red-500/20 transition-all">Test My Emergency Button</motion.button>
               </motion.div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed bottom-[5.5rem] left-0 right-0 flex justify-center z-40 pointer-events-none md:hidden">
            ...
            >
              ...
              <motion.div animate={{ opacity: [0.2, 0.5, 0.2] }} transition={{ duration: 2, repeat: Infinity }} className="absolute inset-0 rounded-full bg-red-400/20" />
            </motion.button>
        </div>
      </div>
    </AppLayout>
